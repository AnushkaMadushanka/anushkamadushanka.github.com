...
A product in `work.js` gets a case study page at `/work/<slug>` as soon as you give it a `caseStudy` object, and the route, the card link and the "more work" cross-links all follow. Without one, the card links straight out to the live product instead. Add a route to `scripts/prerender.mjs` at the same time so the new page gets its own static meta.
