...
/* The model sits at head height in world space (y ≈ 1.5–1.86) rather than at
   the origin, so the camera has to be aimed at it explicitly — r3f points its
   default camera at [0,0,0], which aims this one at the floor. */
...
function Head({ onReady }) {
	...
	useEffect(() => {
		...
		/* Ease toward the target in a rAF loop rather than writing rotation
		   straight from the pointer event — smoother, and it decouples the
